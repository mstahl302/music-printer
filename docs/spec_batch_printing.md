...
**Status:** DRAFT — for review. Rev 2: review round 1 applied.
**Date:** 2026-09-01
...
- **Per-file even padding** — to satisfy the rule, each file's effective
  page count (after cover strip) is padded with a blank up to an **even**
  number. An already-even file gets no blank. For a list of one this is
  identical to the single-file rule in
  [specification.md §7.2](specification.md#72-page-set-computation) — it
  subsumes it, so there is one code path.
...
build_plan(files, mode, threshold):
    ...
    for f in files:
        ...
        entries.append(Entry(f, n_src, cover, n_eff, padded))

    total  = sum(e.padded for e in entries)            # always even
    passes = plan_passes(total)                        # no extra global pad
    layout = build_set_layout([e.padded ...], [e.n_eff ...])
    # layout[g] for global page g (1-based) is either
    #   (file_index, source_page)   or   BLANK(file_index)
```

Because every file's `padded` length is even, each file starts at an odd
global index (front-facing ✓), the total is even (no global pad needed),
and any per-file blank lands on an **even** global page — so it prints in
pass 1 and becomes the *back* of that file's last real page.
...
C(1,2,3,4,5,·), every file starting on a sheet front.
...
| Piece | Change |
|---|---|
| `duplex.py` | add `plan_set(effective_lengths) -> SetPlan` — pad each to even, concatenate, call `plan_passes(total)`, build the global layout. For a 1-element list this equals today's single-file plan. |
